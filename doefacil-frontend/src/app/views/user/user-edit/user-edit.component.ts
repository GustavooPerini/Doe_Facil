...
import { AccountService, Me } from '../../../services/account.service';
...
export class UserEditComponent implements OnInit {
  ...
  refresh() {
    ...
    this.account.getMe().subscribe({
      next: (me) => {
        this.me = me;
        this.nameForm.patchValue({ name: me.name, email: me.email });
      },
      error: () => this.err = 'Falha ao carregar seus dados.',
  ...
  saveName() {
    ...
    this.account.updateName(name).subscribe({
      next: (me) => { this.me = me; this.msg = 'Dados atualizados.'; },
      error: () => this.err = 'Não foi possível atualizar seu nome.'
    });
  }
  ...
  changePassword() {
    ...
    this.account.changePassword(currentPassword, newPassword).subscribe({
      next: () => { this.msg = 'Senha alterada.'; this.pwdForm.reset(); },
      error: (e) => this.err = e?.status === 400 ? 'Senha atual incorreta.' : 'Falha ao alterar senha.'
    });
  }
}
