...
export class ItemService {
	...
	list(page = 0, size = 12): Observable<Page<ItemResponse>> {
		...
		return this.http.get<Page<ItemResponse>>(`${this.base}/api/items`, {
			params,
		});
	}

	listMyItems(page = 0, size = 12): Observable<Page<ItemResponse>> {
		const params = new HttpParams().set("page", page).set("size", size);
		return this.http.get<Page<ItemResponse>>(
			`${this.base}/api/items/my-items`,
			{ params }
		);
	}
